...
interface RealTimeColorVisualizerProps {
  onColorSelect?: (_color: { h: number; s: number; b: number; hex: string; rgb: string }) => void
...
export default function RealTimeColorVisualizer({
  ...
  const handleRgbChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parts.length === 3 && parts.every(p => !isNaN(p) && p >= 0 && p <= 255)) {
      const hsb = require('@/lib/utils/colorUtils').rgbToHsb(parts[0], parts[1], parts[2])
      setHue(hsb.h)
      setSaturation(hsb.s)
      setBrightness(hsb.b)
      setInputMode('rgb')
  ...
  // Copy to clipboard
  const copyToClipboard = useCallback((text: string, _format: string) => {
