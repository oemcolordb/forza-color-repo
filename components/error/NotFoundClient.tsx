...
export default function NotFoundClient() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.3) {
        setPaintDrops(prev => [
          ...prev,
          { id: Date.now(), x: Math.random() * 300, y: 0, color: colors[Math.floor(Math.random() * colors.length)] },
        ])
      }
    }, 100)
  ...
  const catchPaint = useCallback(() => {
    ...
    setPaintDrops(prev => {
      const caught = prev.filter(drop => Math.abs(drop.x - carX) < 30 && drop.y > 300 && drop.y < 350)
      if (caught.length > 0) {
  ...
  return (
    <div className="relative min-h-screen text-white flex flex-col items-center justify-center p-4 overflow-hidden">
      <video autoPlay loop muted playsInline suppressHydrationWarning
        className="absolute inset-0 w-full h-full object-cover" src="/EPIC.mp4" />
      <div className="absolute inset-0 bg-black/75" />
      <div className="relative z-10 flex flex-col items-center w-full max-w-2xl px-4">
        <div className="text-center mb-8">
          <h1 className="text-6xl font-bold mb-4 bg-gradient-to-r from-red-500 to-yellow-500 text-transparent bg-clip-text">404</h1>
          <h2 className="text-2xl mb-2">🏁 Lost in the Garage!</h2>
          <p className="text-gray-300 mb-2">Your car got stuck in the paint booth. Help it collect colors to escape!</p>
        ...
        <div className="relative w-80 h-96 bg-gradient-to-b from-gray-700 to-gray-800 rounded-lg border-4 border-gray-600 mb-6 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-b from-gray-600 to-gray-700">
            <div className="absolute top-0 left-0 w-full h-8 bg-gray-500 flex justify-around items-center">
              {[...Array(6)].map((_, i) => (<div key={i} className="w-2 h-4 bg-gray-400 rounded-b"></div>))}
            </div>
          </div>
          {paintDrops.map(drop => (
            <div key={drop.id} className="absolute w-4 h-6 rounded-full animate-pulse"
              style={{ left: `${drop.x}px`, top: `${drop.y}px`, backgroundColor: drop.color, boxShadow: `0 0 10px ${drop.color}` }} />
          ))}
          <div className="absolute bottom-8 w-12 h-8 transition-all duration-200"
            style={{ left: `${carPosition}%`, transform: 'translateX(-50%)' }}>
            <div className="w-full h-full rounded-lg border-2 border-white transition-colors duration-300" style={{ backgroundColor: paintColor }}>
          ...
          {!gameActive && (
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
              <div className="text-center">
                ...
                <button onClick={() => { setGameActive(true); setScore(0); setPaintDrops([]); setPaintColor('#ff0000') }}
        ...
        {gameActive && (
          <div className="flex gap-4 mb-4">
            <button onClick={() => moveCar('left')} className="bg-gray-600 hover:bg-gray-500 px-6 py-2 rounded-lg font-semibold transition-colors">← Left</button>
            <button onClick={() => moveCar('right')} className="bg-gray-600 hover:bg-gray-500 px-6 py-2 rounded-lg font-semibold transition-colors">Right →</button>
          </div>
        )}
        <div className="text-center mb-6">
          <p className="text-lg">🎨 Paint Score: <span className="font-bold text-yellow-400">{score}</span></p>
        ...
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-4">Use the arrow buttons to move your car and catch falling paint drops!</p>
        </div>
      </div>
    </div>
  )
