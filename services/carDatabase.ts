...
class CarDatabaseService {
  ...
  async loadCars(): Promise<void> {
    ...
    try {
      // Try to load optimized data first, fallback to original cars.json
      let carsData: Car[]
      try {
        // First try the optimized data file
        const response = await fetch('/data/cars-optimized.json')
        if (!response.ok) {
          throw new Error(`Failed to fetch optimized data: ${response.status}`)
        }
        const optimizedData = await response.json()
        carsData = optimizedData.cars
        ...
        console.warn('Failed to load optimized data, falling back to cars.json:', error)
        // Fallback to original cars.json
        const response = await fetch('/cars.json')
        if (!response.ok) {
          throw new Error(`Failed to fetch cars.json: ${response.status}`)
        }
        carsData = await response.json()
        console.log(`Loaded ${carsData.length} cars from cars.json`)
