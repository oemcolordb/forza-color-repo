...
// Color name to car model mappings based on automotive knowledge
const colorToModelMappings = {
  ...
  'Guards Red': ['Porsche 911', 'Porsche 944'],
...
function findModelMatches(colorName, make) {
  ...
  const circuits = [
    'nürburgring', 'nurburgring', 'silverstone', 'monza', 'spa', 'le mans', 'lemans',
    'monaco', 'brands hatch', 'goodwood', 'watkins glen', 'laguna seca', 'sebring',
    'daytona', 'indianapolis', 'road america', 'imola', 'mugello', 'suzuka',
    'fuji', 'bathurst', 'nordschleife', 'paul ricard', 'hockenheim', 'interlagos'
  ];
  ...
  if (lowerColorName.includes('gt') || lowerColorName.includes('sport') || lowerColorName.includes('performance')) {
    switch (lowerMake) {
      case 'porsche':
  ...
  // Type R colors
  if (lowerColorName.includes('type r') || (lowerMake === 'honda' && lowerColorName.includes('championship'))) {
    matches.push('Honda Civic Type R');
  }
  
  // AMG colors
  if (lowerColorName.includes('amg') || (lowerMake === 'mercedes' && lowerColorName.includes('magno'))) {
    matches.push('Mercedes AMG GT', 'Mercedes C63 AMG', 'Mercedes E63 AMG');
  }
  
  // M colors
  if ((lowerMake === 'bmw' && lowerColorName.includes('m ')) || lowerColorName.includes('competition')) {
    matches.push('BMW M3', 'BMW M4', 'BMW M5');
  }
...
// Filter only colors that have model matches
const colorsWithMatches = processedData.filter(color => color.suggestedModels && color.suggestedModels.length > 0);

// Group by manufacturer
const groupedByMake = colorsWithMatches.reduce((acc, color) => {
  if (!acc[color.make]) {
    acc[color.make] = [];
  }
  acc[color.make].push(color);
  return acc;
}, {});

// Generate report
console.log('='.repeat(80));
console.log('COLOR TO MODEL MATCHING REPORT');
console.log('='.repeat(80));
console.log();

let totalMatches = 0;
Object.keys(groupedByMake).sort().forEach(make => {
  const colors = groupedByMake[make];
  console.log(`${make.toUpperCase()} (${colors.length} colors with model matches)`);
  console.log('-'.repeat(50));
  
  colors.forEach(color => {
    console.log(`  ${color.colorName}`);
    console.log(`    Suggested Models: ${color.suggestedModels.join(', ')}`);
    console.log(`    Color Type: ${color.colorType}`);
    console.log();
    totalMatches++;
  });
  console.log();
...
console.log('='.repeat(80));
console.log(`SUMMARY: Found ${totalMatches} colors with model matches out of ${colorData.length} total colors`);
console.log(`Match Rate: ${((totalMatches / colorData.length) * 100).toFixed(2)}%`);
console.log('='.repeat(80));

// Save a summary report
const reportContent = `# Color to Model Matching Report

## Summary
- Total colors analyzed: ${colorData.length}
- Colors with model matches: ${totalMatches}
- Match rate: ${((totalMatches / colorData.length) * 100).toFixed(2)}%

## Matches by Manufacturer

${Object.keys(groupedByMake).sort().map(make => {
  const colors = groupedByMake[make];
  return `### ${make} (${colors.length} matches)

${colors.map(color => `- **${color.colorName}**
  - Models: ${color.suggestedModels.join(', ')}
  - Type: ${color.colorType}`).join('\n\n')}`;
}).join('\n\n')}

## Methodology
This analysis matched color names to car models based on:
1. Direct color name mappings from automotive knowledge
2. Racing circuit names in color names
3. Performance/special edition indicators (GT, RS, AMG, Type R, etc.)
4. Championship and racing color patterns
5. Manufacturer-specific naming conventions
`;

fs.writeFileSync(
  path.join(__dirname, '..', 'COLOR_MODEL_MATCHES.md'),
  reportContent
);

console.log('\nDetailed report saved to COLOR_MODEL_MATCHES.md');