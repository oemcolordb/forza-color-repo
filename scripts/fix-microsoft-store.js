...
class MicrosoftStoreFix {
  ...
  static async checkFirewall() {
    try {
      const { stdout } = await execAsync('netsh advfirewall firewall show rule name="Forza Telemetry"')
  ...
  static async addFirewallRule() {
    const cmd = 'netsh advfirewall firewall add rule name="Forza Telemetry" dir=in action=allow protocol=UDP localport=9999'
    try {
      await execAsync(cmd)
