...
def parse_tune_sheet(pdf_path):
    ...
    for line in lines:
        ...
        if current_car:
            ...
            if share_code:
                share_idx = text.find(share_code)
                pi_value = extract_pi_value(text[share_idx:share_idx+50])
...
def main():
    ...
    # Find all tune-related PDFs - expanded keywords
    tune_keywords = [
    ...
    for f in os.listdir(research_folder):
        if f.endswith('.pdf'):
            ...
            if any(keyword in f_lower for keyword in tune_keywords):
                ...
                # Catch other Forza/FH5 related PDFs
                pdf_files.append(os.path.join(research_folder, f))
