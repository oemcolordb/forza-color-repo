...
export const TOP_MANUFACTURERS = ${JSON.stringify(
  ...
  })).sort((a, b) => b.count - a.count).slice(0, 10), null, 2
)};
...
`;
    ...
    fs.writeFileSync(
      path.join(outputDir, 'carStats.ts'),
      interfaceContent
    );
    ...
    fs.writeFileSync(
      path.join(outputDir, 'car-types.json'),
    ...
    fs.writeFileSync(
      path.join(outputDir, 'cars-sample.json'),
      JSON.stringify(sampleCars, null, 2)
    );
    ...
    console.log(`   - Price range: $${metadata.priceRange.min.toLocaleString()} - $${metadata.priceRange.max.toLocaleString()}`);
