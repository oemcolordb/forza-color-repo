...
export class FH6Calculator {
  ...
  calculateTirePressure(position) {
    if (this.piValue >= 900) return 29.0
    if (this.piValue >= 800) return 28.0
    return 25.0
  }
  ...
  calculateToe(position) {
    if (position === 'front' && this.drivetrain === 'FWD') return 0.1
    if (position === 'rear' && this.drivetrain === 'RWD') return -0.1
    return 0.0
  }
  ...
  calculateDamping(type, position) {
    const w = position === 'front' ? this.frontWeight : this.rearWeight
    const cornerLoad = (this.weight * w) / 2
    const cornerLoadLbs = cornerLoad * 2.20462
    const baseBump = 4.5
    const bump = baseBump + (cornerLoadLbs / 200) * 0.1
    const rebound = bump / 0.4
  ...
  calculateDifferential(type) {
    if (this.drivetrain === 'RWD') {
      return type === 'accel' ? 55 : 15
    }
    if (this.drivetrain === 'FWD') {
      return type === 'accel' ? 85 : 0
    }
    return type === 'accel' ? 55 : 15
  }
