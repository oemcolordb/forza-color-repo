...
export async function getColorData(): Promise<CarColor[]> {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Note: extracted_colors.json is intentionally NOT fetched on the client —
    // it adds ~262 KB of mostly duplicate data that gets removed by dedupeColors().
    // Server-side still reads both for completeness.
    try {
