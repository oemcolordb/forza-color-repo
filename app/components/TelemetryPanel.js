      ...
      () => {
        setTelemetryData(prev => {
          ...
          return {
            ...prev,
            tireTemp: {
              front: prev.tireTemp.front.map(temp =>
                Math.max(60, Math.min(120, temp + (Math.random() - 0.5) * 4 * intensity))
              ),
              rear: prev.tireTemp.rear.map(temp =>
                Math.max(60, Math.min(120, temp + (Math.random() - 0.5) * 4 * intensity))
              ),
            },
            engine: {
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-sm font-medium ${
              isRacing ? 'bg-red-500 text-white animate-pulse' : 'bg-green-500 text-white'
            }`}
          >
              ...
              {['FL', 'FR', 'RL', 'RR'].map((corner, i) => {
                ...
                return (
                  <div key={corner} className="text-center">
                    ...
                    <div className="mt-1">
                      <div
                        className={`h-2 rounded ${status.color}`}
                        style={{ width: `${compression}%` }}
                      ></div>
                      <div className="text-xs mt-1">
                        {compression.toFixed(0)}% - {status.status}
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`font-mono text-lg ${
                    telemetryData.engine.rpm > 7000
                      ? 'text-red-400'
                      : telemetryData.engine.rpm > 5000
                        ? 'text-yellow-400'
                        : 'text-green-400'
                  }`}
                >
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-lg text-purple-400">
                  {telemetryData.engine.boost} bar
                </span>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="w-full bg-gray-600 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${telemetryData.engine.throttle}%` }}
                  ></div>
                </div>
              </div>
              <div>
                ...
                <div className="w-full bg-gray-600 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${telemetryData.engine.brake}%` }}
                  ></div>
                </div>
              </div>
            </div>
