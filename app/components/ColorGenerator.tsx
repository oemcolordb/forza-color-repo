...
interface ColorGeneratorProps {
  colors: CarColor[]
  isDarkMode: boolean
  onColorsGenerated: (colors: CarColor[]) => void
  isMobile?: boolean
}

const ColorGenerator: React.FC<ColorGeneratorProps> = ({ colors, isDarkMode, onColorsGenerated, isMobile = false }) => {
  const [isGenerating, setIsGenerating] = useState(false)
  const [generatedCount, setGeneratedCount] = useState(0)

  // Generate systematic color variants using mathematical progression
  const generateSystematicVariants = useCallback((baseColors: CarColor[], type: string) => {
    ...
    const processInChunks = (colors: CarColor[], chunkSize = 500) => {
  ...
  const generateSpecificType = useCallback(
    async (type: string) => {
  ...
  const getVariantCount = (_type: string) => {
  ...
  return (
    ...
    >
      <h3
        className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold ${isMobile ? 'mb-3' : 'mb-4'} ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
      >
        🧬 Systematic Color Generator
      </h3>

      <div
        className={`${isMobile ? 'mb-3 p-2' : 'mb-4 p-3'} rounded ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}
      >
        <h4
          className={`font-medium ${isMobile ? 'text-xs' : 'text-sm'} ${isMobile ? 'mb-1' : 'mb-2'} ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
        >
          Generation Stats
        </h4>
        <div
          className={`grid grid-cols-2 ${isMobile ? 'gap-1' : 'gap-2'} ${isMobile ? 'text-2xs' : 'text-xs'}`}
        >
          <div>
          ...
          <div>
            ...
            <span
              className={`ml-1 font-medium ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}
            >
      ...
      <div className={`${isMobile ? 'space-y-1 mb-3' : 'space-y-2 mb-4'}`}>
        <div
          className={`${isMobile ? 'text-2xs' : 'text-xs'} ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}
        >
          Systematic Generation:
        </div>
        <div
          className={`grid ${isMobile ? 'grid-cols-1 gap-1' : 'grid-cols-2 gap-1'} ${isMobile ? 'text-2xs' : 'text-xs'}`}
        >
          <button
          ...
          >
            ...
            <div className={isDarkMode ? 'text-slate-400' : 'text-red-600'}>
              {getVariantCount('hue').toLocaleString()} colors
            </div>
          </button>
          ...
          >
            <div className={`font-medium ${isDarkMode ? 'text-yellow-400' : 'text-yellow-700'}`}>
              ⚪ Brightness
            </div>
            <div className={isDarkMode ? 'text-slate-400' : 'text-yellow-600'}>
              {getVariantCount('brightness').toLocaleString()} colors
            </div>
          </button>
          ...
          >
            ...
            <div className={isDarkMode ? 'text-slate-400' : 'text-purple-600'}>
              {getVariantCount('saturation').toLocaleString()} colors
            </div>
          </button>
          ...
          >
            <div className={`font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-700'}`}>
              ✨ Finishes
            </div>
            <div className={isDarkMode ? 'text-slate-400' : 'text-blue-600'}>
              {getVariantCount('finishes').toLocaleString()} colors
            </div>
          </button>
        </div>
      </div>
      ...
      {generatedCount > 0 && (
        <div
          className={`${isMobile ? 'mt-2 p-1.5' : 'mt-3 p-2'} rounded ${isMobile ? 'text-2xs' : 'text-xs'} text-center ${isDarkMode ? 'bg-green-900 text-green-300' : 'bg-green-100 text-green-800'}`}
        >
          ✅ Generated {generatedCount.toLocaleString()} colors! Systematic color space exploration
          complete.
        </div>
      )}
    </div>
