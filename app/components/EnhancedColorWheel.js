...
const EnhancedColorWheel = ({ currentHarmony, harmonyMode, isDarkMode, onColorSelect }) => {
  ...
  if (!currentHarmony || currentHarmony.length === 0) {
    return (
      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg h-full`}>
        ...
        {showHelp && (
          <div className={`mb-4 p-3 rounded text-sm ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-blue-50 text-blue-800'}`}>
  ...
  return (
    <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg h-full`}>
      ...
      {/* Harmony explanation */}
      <div className={`mb-3 p-2 rounded text-xs ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-blue-50 text-blue-700'}`}>
      ...
      {showHelp && (
        <div className={`mb-4 p-3 rounded text-sm ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-blue-50 text-blue-800'}`}>
          ...
          <ul className="space-y-1 text-xs">
            <li>• <span className="inline-block w-3 h-3 bg-white border rounded-full mr-1"></span>White border = base color</li>
            <li>• <span className="inline-block w-3 h-3 bg-gray-400 rounded-full mr-1"></span>Dotted lines = color relationships</li>
            <li>• <span className="inline-block w-3 h-3 bg-gradient-to-r from-red-500 to-blue-500 rounded-full mr-1"></span>Outer ring = hue spectrum (0-360°)</li>
      ...
      <div className="flex flex-col items-center h-full">
        ...
        <div className="relative mb-6">
          <svg width="240" height="240" viewBox="0 0 240 240" className="transform -rotate-90">
            ...
            <circle cx="120" cy="120" r="110" fill="none" stroke="url(#hueGradient)" strokeWidth="20" opacity="0.3" />
            <circle cx="120" cy="120" r="100" fill="none" stroke={isDarkMode ? '#374151' : '#d1d5db'} strokeWidth="2" />
            ...
            {[0, 90, 180, 270].map(degree => {
              ...
              return (
                <g key={degree}>
                  <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={isDarkMode ? '#6b7280' : '#9ca3af'} strokeWidth="2" />
                  <text x={textX} y={textY} textAnchor="middle" className={`text-xs ${isDarkMode ? 'fill-gray-400' : 'fill-gray-500'}`} transform={`rotate(90 ${textX} ${textY})`}>
                    {degree}°
                  </text>
                </g>
              )
            ...
            {currentHarmony.length > 1 && !['Random Roulette', 'Brand Harmony', 'Warm Temperature', 'Cool Temperature', 'High Saturation', 'Medium Saturation', 'Low Saturation', 'Light Brightness', 'Medium Brightness', 'Dark Brightness', 'Spring Palette', 'Summer Palette', 'Fall Palette', 'Winter Palette'].includes(harmonyMode) && (
              <g>
            ...
            {currentHarmony.map((color, index) => {
              ...
              const overlaps = currentHarmony.filter((c, i) => 
                i !== index && Math.abs((c.color1.h * 360) - (color.color1.h * 360)) < 15
              )
              ...
              return (
                <g key={index} className="cursor-pointer" onClick={() => onColorSelect?.(color)}>
                  ...
                  />
                  {/* Color number */}
                  <text x={x} y={y + 25} textAnchor="middle" className={`text-xs font-bold ${isDarkMode ? 'fill-white' : 'fill-gray-900'} pointer-events-none`} transform={`rotate(90 ${x} ${y + 25})`}>
                    {index + 1}
                  </text>
                </g>
              )
            ...
            {/* Center label */}
            <circle cx="120" cy="120" r="25" fill={isDarkMode ? '#1f2937' : '#f9fafb'} stroke={isDarkMode ? '#374151' : '#d1d5db'} strokeWidth="2" />
            <text x="120" y="125" textAnchor="middle" className={`text-xs font-medium ${isDarkMode ? 'fill-white' : 'fill-gray-900'}`} transform="rotate(90 120 120)">
              {harmonyMode.split(' ')[0]}
            </text>
          </svg>
        </div>
            ...
            {currentHarmony.map((color, index) => (
              ...
              >
                <div className="relative">
                  ...
                  />
                  {index === 0 && (
                    <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${isDarkMode ? 'bg-blue-400' : 'bg-blue-500'} text-white text-xs flex items-center justify-center`}>
                      1
                    </div>
                  )}
                </div>
