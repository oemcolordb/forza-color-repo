...
const VirtualizedColorGrid: React.FC<VirtualizedColorGridProps> = React.memo(({
  ...
  const processedColors = useMemo(() => {
    ...
    switch (sortBy) {
      case 'manufacturer':
        sorted.sort((a, b) => a.make.localeCompare(b.make) || a.colorName.localeCompare(b.colorName))
        break
      case 'hue':
  ...
  const selectAll = useCallback(() => {
    setSelectedColors(new Set(processedColors.map(c => `${c.make}-${c.colorName}-${c.year || 'unknown'}`)))
  }, [processedColors])
