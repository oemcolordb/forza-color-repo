...
const HARMONY_MODES: Record<string, { icon: string; desc: string }> = {
...
interface ColorRouletteHarmonyProps {
  ...
  onHarmonyGenerated: (colors: CarColor[], mode: string) => void
}

const ColorRouletteHarmony: React.FC<ColorRouletteHarmonyProps> = ({ colors, isDarkMode, onColorSelect, onHarmonyGenerated }) => {
  ...
  const isWarmColor = useCallback((hue: number) => (hue >= 0 && hue <= 60) || (hue >= 300 && hue <= 360), [])
  const isCoolColor = useCallback((hue: number) => hue >= 120 && hue <= 300, [])

  const getSeasonalColors = useCallback((season: string, colors: CarColor[]) => {
    return colors.filter(color => {
