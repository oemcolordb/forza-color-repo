...
const ColorAnalytics = ({ colors, isDarkMode, selectedMake, selectedColorType }) => {
  ...
  if (!pythonApiAvailable) {
    return (
      <div
        className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800 text-slate-300' : 'bg-gray-100 text-gray-700'}`}
      >
        <p className="text-sm">
  ...
  if (error) {
    return (
      <div
        className={`p-4 rounded-lg ${isDarkMode ? 'bg-red-900/30 text-red-200' : 'bg-red-50 text-red-800'}`}
      >
  ...
  return (
    <div className={`p-6 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg`}>
      ...
      <div className="mb-6">
        <h4
          className={`text-lg font-semibold mb-3 ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}
        >
          Color Type Distribution
        </h4>
            ...
            .map(([type, data]) => (
              <div key={type} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <div
                    className={`w-24 h-2 rounded-full ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`}
                  >
                    <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {Object.entries(analysis.manufacturer_analysis)
            ...
            .map(([make, data]) => (
              <div
                key={make}
                className={`p-3 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}
              >
      ...
      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <span
              className={`block font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}
            >
              Harmony Score
            </span>
            <span
              className={`text-lg font-bold ${getHarmonyColor(analysis.harmony_analysis.harmony_score)}`}
            >
          ...
          <div>
            ...
            <span
              className={`text-lg font-bold ${isDarkMode ? 'text-slate-200' : 'text-gray-800'}`}
            >
...
// Helper components
const StatCard = ({ title, value, subtitle, isDarkMode }) => (
  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
...
const PerceptualStat = ({ title, stats, isDarkMode }) => (
  <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
    ...
    <div className="space-y-1 text-xs">
      ...
      <div className={`flex justify-between ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
        <span>Range:</span>
        <span>
          {stats.range[0].toFixed(1)} - {stats.range[1].toFixed(1)}
        </span>
      </div>
    </div>
  </div>
