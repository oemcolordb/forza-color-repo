...
function estimateBrakeTemp(prev: number | null, data: TelemetryData): number {
...
interface TelemetryData {
  ...
  suspensionTravelNormalized: number[]
...
interface TelemetryState {
  ...
  lapCoords: [number, number][]
  prevLapCoords: [number, number][]
  lapNumber: number
...
type TelemetryAction =
  | { type: 'CONNECT' }
  | { type: 'DISCONNECT' }
  | { type: 'UPDATE'; payload: TelemetryData }
...
function telemetryReducer(state: TelemetryState, action: TelemetryAction): TelemetryState {
  switch (action.type) {
    ...
    case 'UPDATE': {
      ...
      const newCoord: [number, number] = [d.positionX, d.positionZ]
...
const StatCard = React.memo(function StatCard({ label, value, sub, accent }: { label: string; value: string | number; sub?: string; accent?: string }) {
  return (
...
const RpmBar = React.memo(function RpmBar({ rpm, maxRpm }: { rpm: number; maxRpm: number }) {
...
const GForcePanel = React.memo(function GForcePanel({ ax, az }: { ax: number; az: number }) {
  const lat   = ax / G
  const lon   = az / G
...
const BoostGauge = React.memo(function BoostGauge({ boost }: { boost: number }) {
...
const FuelPanel = React.memo(function FuelPanel({ fuel, carClass }: { fuel: number; carClass: number }) {
...
const TirePanel = React.memo(function TirePanel({ tireTemp, tireWear, tireSlipRatio, brakeTemp }: { tireTemp: number[]; tireWear: number[]; tireSlipRatio: number[]; brakeTemp: number[] }) {
...
const LapDelta = React.memo(function LapDelta({ currentLap, bestLap, sessionBestLap }: { currentLap: number; bestLap: number; sessionBestLap: number | null }) {
...
const BrakeAnalysis = React.memo(function BrakeAnalysis({ brake, brakeTemp, tireSlipRatio }: { brake: number; brakeTemp: number[]; tireSlipRatio: number[] }) {
...
const SuspensionPanel = React.memo(function SuspensionPanel({ normalized, meters }: { normalized: number[]; meters: number[] }) {
  return (
    <div className="p-3 rounded bamboo-surface-dark">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="p-3 rounded bamboo-surface-dark">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between"><span className="text-white/55">Session best</span><span className="text-green-400">{formatTime(sessionBestLap ?? 0)}</span></div>
