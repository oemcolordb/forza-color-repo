...
const HarmonyVisualizer = ({ currentHarmony, harmonyMode, isDarkMode, onColorSelect }) => {
  ...
  if (!currentHarmony || currentHarmony.length === 0) {
    return (
      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg h-full flex items-center justify-center`}>
    ...
    <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg h-full`}>
      ...
      <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
        <div className="flex items-center justify-between">
          <span>Hue Range:</span>
          <span>
            {Math.min(...currentHarmony.map(c => Math.round(c.color1.h * 360)))}° - {Math.max(...currentHarmony.map(c => Math.round(c.color1.h * 360)))}°
          </span>
        </div>
      </div>
    </div>
