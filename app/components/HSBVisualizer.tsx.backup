...
interface HSBVisualizerProps {
  colors: CarColor[]
  isDarkMode: boolean
}

const HSBVisualizer: React.FC<HSBVisualizerProps> = ({ colors, isDarkMode }) => {
  const [selectedAxis, setSelectedAxis] = useState<'hue' | 'saturation' | 'brightness'>('hue')
  ...
  const getAxisValue = (color: CarColor, axis: string) => {
    switch (axis) {
      case 'hue': return color.color1.h * 360
      case 'saturation': return color.color1.s * 100
      case 'brightness': return color.color1.b * 100
      default: return 0
    }
  }
    ...
    <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg`}>
      ...
      <div className="mb-4">
        <div className="flex gap-2 mb-3">
          {(['hue', 'saturation', 'brightness'] as const).map(axis => (
            <button
              key={axis}
      ...
      <div className="grid grid-cols-10 gap-1 mb-4">
        {sortedColors.map((color, index) => (
          <div
            ...
            style={{ backgroundColor: hsbToHex(color.color1.h * 360, color.color1.s, color.color1.b) }}
      ...
      <div className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
        <p>Showing {sortedColors.length} colors sorted by {selectedAxis}</p>
        <p className="text-xs mt-1">
          Range: {Math.round(getAxisValue(sortedColors[0], selectedAxis))} - {Math.round(getAxisValue(sortedColors[sortedColors.length - 1], selectedAxis))}{selectedAxis === 'hue' ? '°' : '%'}
        </p>
      </div>
    </div>
  )
