...
interface MobileColorStatsProps {
  colors: CarColor[]
  favorites: string[]
  colorHistory: string[]
  isDarkMode: boolean
...
const MobileColorStats: React.FC<MobileColorStatsProps> = ({ colors, favorites, colorHistory, isDarkMode }) => {
  const stats = useMemo(() => {
    const makeCount = colors.reduce<Record<string, number>>((acc, color) => {
