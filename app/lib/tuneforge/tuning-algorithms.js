...
import {
  CAR_TYPE_DEFAULTS,
  DRIVETRAIN_MODIFIERS,
  PI_CLASS_MODIFIERS,
  GEAR_RATIO_BLUEPRINTS,
} from './types.js'
...
export class TuningAlgorithms {
  static getCarDefaults(car) {
    ...
    if (baseDefaults['springs-front'])
      baseDefaults['springs-front'] = Math.round(baseDefaults['springs-front'] * piMods.springs)
    if (baseDefaults['springs-rear'])
      baseDefaults['springs-rear'] = Math.round(baseDefaults['springs-rear'] * piMods.springs)
    if (baseDefaults['tire-pressure-front'])
      baseDefaults['tire-pressure-front'] =
        Math.round(baseDefaults['tire-pressure-front'] * piMods.pressure * 10) / 10
    if (baseDefaults['tire-pressure-rear'])
      baseDefaults['tire-pressure-rear'] =
        Math.round(baseDefaults['tire-pressure-rear'] * piMods.pressure * 10) / 10
