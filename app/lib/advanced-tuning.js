...
export class AdvancedTuningCalculator extends TuningCalculator {
  ...
  getWeatherRecommendations(weather, baseTune) {
    ...
    switch (weather) {
      case 'wet':
        ...
        adjustments['differential-rear-accel'] = Math.max(
          baseTune['differential-rear-accel'] - 15,
          5
        )
        break
  ...
  predictLapTimeImprovement(currentSetup, newSetup) {
    const improvements = {
      camber:
        Math.abs((newSetup['camber-front'] || 0) - (currentSetup['camber-front'] || 0)) * 0.15,
      pressure:
        Math.abs(
          (newSetup['tire-pressure-front'] || 0) - (currentSetup['tire-pressure-front'] || 0)
        ) * 0.08,
      aero: Math.abs((newSetup['aero-rear'] || 0) - (currentSetup['aero-rear'] || 0)) * 0.003,
      springs:
        Math.abs((newSetup['spring-rate-front'] || 0) - (currentSetup['spring-rate-front'] || 0)) *
        0.002,
      differential:
        Math.abs(
          (newSetup['differential-rear-accel'] || 0) -
            (currentSetup['differential-rear-accel'] || 0)
        ) * 0.01,
    }
  ...
  getParameterImpact(parameter, change) {
    const impacts = {
      'tire-pressure-front':
        change > 0 ? 'Reduced grip, better tire wear' : 'Increased grip, faster wear',
      'camber-front': change < 0 ? 'Better cornering grip' : 'More stable, less aggressive',
      'anti-roll-bar-front':
        change > 0 ? 'Stiffer front end, less understeer' : 'Softer front, more grip',
      'differential-rear-accel':
        change > 0 ? 'Better traction, less wheelspin' : 'More rotation, easier slides',
    }
