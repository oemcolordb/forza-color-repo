...
const getMarkerColor = (type: LocationType) => {
  switch (type) {
    case LocationType.Parking: return 'bg-blue-500'
    case LocationType.PhotoOp: return 'bg-purple-500'
    case LocationType.Landmark: return 'bg-green-500'
    case LocationType.ScenicView: return 'bg-yellow-500'
    default: return 'bg-gray-500'
  }
}

const MapDisplay: React.FC<MapDisplayProps> = ({ selectedLocation, allLocations, onLocationSelect }) => {
    ...
    <div className="w-full h-full flex flex-col bg-gray-900">
      ...
      <div className="p-3 bg-gradient-to-r from-gray-800 to-gray-900 border-b border-gray-700 shadow-lg">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            <button onClick={() => setZoom(Math.min(zoom + 0.5, 4))} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-bold shadow-lg transition-all hover:scale-105">+</button>
            <button onClick={() => setZoom(Math.max(zoom - 0.5, 0.5))} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-bold shadow-lg transition-all hover:scale-105">−</button>
            <button onClick={() => { setZoom(1); setPosition({ x: 50, y: 50 }) }} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-white text-sm font-semibold shadow-lg transition-all">Reset</button>
          </div>
        </div>
      </div>
        ...
        {selectedLocation && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 pointer-events-none z-50">
            <div className="bg-gradient-to-r from-red-600 via-orange-600 to-red-600 px-6 py-3 rounded-xl shadow-2xl border-4 border-white animate-pulse">
              <div className="text-white font-bold text-xl text-center">📍 {selectedLocation.name}</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-3 bg-gray-800/95 border-t border-gray-700">
        <div className="flex items-center justify-between text-xs text-gray-400">
          ...
          <span>{allLocations.filter(l => filters.includes(l.type)).length} locations available</span>
        </div>
      </div>
    </div>
  )
